...
async function getClaimParams(options) {
  const cc = await (async () => {
    if (options.type === "erc1155") {
      const { getActiveClaimCondition: getActiveClaimCondition2 } = await import("./getActiveClaimCondition-UZFHLDGQ.js");
    ...
    if (options.type === "erc721") {
      if (options.singlePhaseDrop) {
        const { claimCondition } = await import("./claimCondition-VDXS2FW4.js");
        return await claimCondition({
          contract: options.contract
        });
      }
      const { getActiveClaimCondition: getActiveClaimCondition2 } = await import("./getActiveClaimCondition-2H5AGRCQ.js");
      return await getActiveClaimCondition2({
        contract: options.contract
      });
    }
    ...
    const { getActiveClaimCondition } = await import("./getActiveClaimCondition-AZJUTM6K.js");
  ...
  const allowlistProof = await (async () => {
    ...
    const { fetchProofsForClaimer } = await import("./fetch-proofs-for-claimers-SLIBHWLQ.js");
...
//# sourceMappingURL=chunk-2CJKMOMZ.js.map
